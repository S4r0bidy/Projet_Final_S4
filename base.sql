...
-- Barème de frais par tranche de montant, par type d'opération
...
-- Comptes clients (login automatique par numéro)
CREATE TABLE comptes_clients (
    id                    INTEGER PRIMARY KEY AUTOINCREMENT,
    numero_telephone      TEXT NOT NULL UNIQUE,
    prefixe_id            INTEGER,             -- déduit du numéro à la création du compte
...
CREATE TABLE operations (
    ...
    type_operation_id       INTEGER NOT NULL,
    compte_source_id        INTEGER,          -- NULL pour un dépôt (argent externe)
    compte_destination_id   INTEGER,          -- NULL pour un retrait
    bareme_id               INTEGER,          -- tranche de frais appliquée
...
-- Gains de l'opérateur : total des frais perçus, par type d'opération
...
-- Historique lisible des opérations (avec libellés)
...
-- Barème de frais — RETRAIT (type_operation_id = 2)
...
-- Barème de frais — TRANSFERT (type_operation_id = 3)
INSERT INTO baremes_frais (type_operation_id, montant_min, montant_max, frais) VALUES
(3,     100,    5000,     50),
(3,    5001,   20000,    200),
(3,   20001,   50000,    500),
...
-- Comptes clients de test (prefixe_id déduit des 3 premiers chiffres du numéro)
