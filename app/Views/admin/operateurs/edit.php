  ...
  <div class="row justify-content-center">
    <div class="col-md-6 col-lg-5">
      <div class="card shadow-sm p-4">
        <h2 class="mb-4">Modifier l'opérateur</h2>

        <?php if (session('error')): ?>
          <div class="alert alert-danger"><?= esc(session('error')) ?></div>
        <?php endif; ?>

        <form method="post" action="/admin/operateurs/<?= (int)$operateur['id'] ?>/update">
          <div class="mb-3">
            <label class="form-label">Nom de l'opérateur</label>
            <input name="nom" class="form-control" required value="<?= esc($operateur['nom']) ?>" placeholder="Orange, Airtel, Yas...">
          </div>

          <div class="d-flex gap-2">
            <button type="submit" class="btn btn-primary">Enregistrer</button>
            <a href="/admin/operateurs" class="btn btn-outline-secondary">Annuler</a>
          </div>
        </form>
      </div>
  </div>
